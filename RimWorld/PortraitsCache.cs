...
public static class PortraitsCache
{
	...
	private readonly struct PortraitParams
	{
		...
		public unsafe PortraitParams(Vector2 size, Vector3 cameraOffset, float cameraZoom, Rot4 rotation, bool renderHeadgear = true, bool renderClothes = true, IReadOnlyDictionary<Apparel, Color> overrideApparelColors = null, Color? overrideHairColor = null, bool stylingStation = false, PawnHealthState? overrideHealthState = null)
		{
			...
			cachedHashCode = ((object)(*(Vector2*)(&size))/*cast due to .constrained prefix*/).GetHashCode() ^ ((object)(*(Vector3*)(&cameraOffset))/*cast due to .constrained prefix*/).GetHashCode() ^ cameraZoom.GetHashCode() ^ rotation.GetHashCode() ^ renderHeadgear.GetHashCode() ^ renderClothes.GetHashCode() ^ stylingStation.GetHashCode() ^ (overrideHairColor.HasValue ? ((object)overrideHairColor.GetValueOrDefault()/*cast due to .constrained prefix*/).GetHashCode() : 0) ^ (overrideHealthState?.GetHashCode() ?? 0) ^ GenCollection.DictHashCode(overrideApparelColors);
		}
