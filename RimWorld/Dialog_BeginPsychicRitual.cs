...
public class Dialog_BeginPsychicRitual : Dialog_BeginLordJob
{
	...
	public override void DrawExtraOutcomeDescriptions(Rect viewRect, FloatRange qualityRange, string qualityNumber, ref float curY, ref float totalInfoHeight)
	{
		...
		float num = Text.CalcHeight(label.Resolve(), ((Rect)(ref viewRect)).width);
		Widgets.Label(new Rect(((Rect)(ref viewRect)).x, curY, ((Rect)(ref viewRect)).width, num), label);
		curY += num;
		totalInfoHeight += num;
		foreach (TaggedString item in psychicRitualDef.OutcomeWarnings(assignments))
		{
			float num2 = Math.Max(Text.CalcHeight(item, ((Rect)(ref viewRect)).width) + 3f, 28f);
			Widgets.Label(new Rect(((Rect)(ref viewRect)).x, curY + 4f, ((Rect)(ref viewRect)).width, num2), item);
			curY += num2;
		...
		if (list.Count > 0)
		{
			...
			float num3 = Math.Max(Text.CalcHeight(taggedString, ((Rect)(ref viewRect)).width) + 3f, 28f);
			Widgets.Label(new Rect(((Rect)(ref viewRect)).x, curY + 4f, ((Rect)(ref viewRect)).width, num3), taggedString);
			curY += num3;
		...
		if (list2.Count > 0)
		{
			...
			float num4 = Math.Max(Text.CalcHeight(taggedString2, ((Rect)(ref viewRect)).width) + 3f, 28f);
			Widgets.Label(new Rect(((Rect)(ref viewRect)).x, curY + 4f, ((Rect)(ref viewRect)).width, num4), taggedString2);
			curY += num4;
