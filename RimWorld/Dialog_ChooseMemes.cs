	...
	private void DoStructureMemeSelector(Rect viewRect, ref float curY, List<MemeDef> memes)
	{
		...
		for (int num5 = 0; num5 < memes.Count; num5++)
		{
			...
			else
			{
				...
				float num9 = (((Rect)(ref viewRect)).width - (float)num8 * IdeoUIUtility.MemeBoxSize.x - (float)(num8 - 1) * num3) / 2f;
				((Rect)(ref memeBox))._002Ector(num9 + (float)num7 * IdeoUIUtility.MemeBoxSize.x + (float)num7 * num3, curY + (float)num6 * IdeoUIUtility.MemeBoxSize.y + (float)num6 * num3, IdeoUIUtility.MemeBoxSize.x, IdeoUIUtility.MemeBoxSize.y);
		...
		if (memeGroups.Any())
		{
			...
			{
				Rect val = DrawMemeGroup(memeGroup.Value, new Vector2(((Rect)(ref viewRect)).x + ((Rect)(ref viewRect)).width * memeGroup.Key.drawOffset.x, ((Rect)(ref viewRect)).y + num10 + ((Rect)(ref viewRect)).height * memeGroup.Key.drawOffset.y), memeGroup.Key.maxRows);
