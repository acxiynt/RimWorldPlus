...
public class ITab_ContentsMapPortal : ITab_ContentsBase
{
	...
	protected override void DoItemsLists(Rect inRect, ref float curY)
	{
		...
		((Rect)(ref viewRect)).y = 0f;
		((Rect)(ref viewRect)).height = lastHeight;
		if (lastHeight > ((Rect)(ref inRect)).height)
		{
			((Rect)(ref viewRect)).width = ((Rect)(ref viewRect)).width - 16f;
		}
		Widgets.BeginScrollView(inRect, ref scroll, viewRect);
		Widgets.ListSeparator(ref curY2, ((Rect)(ref inRect)).width, Portal.EnteringString);
		if (portal.leftToLoad != null)
		{
			for (int i = 0; i < portal.leftToLoad.Count; i++)
			{
				...
				{
					flag = true;
					DoThingRow(t.ThingDef, t.CountToTransfer, t.things, ((Rect)(ref viewRect)).width, ref curY2, delegate(int x)
					{
		...
		if (!flag)
		{
			Widgets.NoneLabel(ref curY2, ((Rect)(ref inRect)).width);
		}
		Widgets.EndGroup();
	}
