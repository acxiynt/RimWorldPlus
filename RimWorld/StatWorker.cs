...
public class StatWorker
{
	...
	public virtual string GetExplanationUnfinalized(StatRequest req, ToStringNumberSense numberSense)
	{
		...
		if (pawn != null)
		{
			...
			for (int num4 = 0; num4 < hediffs.Count; num4++)
			{
				...
				if (num5 != 0f)
				{
					...
					stringBuilder.Append("    " + hediffs[num4].LabelBaseCap + ": " + ValueToString(num5, finalized: false, ToStringNumberSense.Offset));
					if (curStage.statOffsetEffectMultiplier != null)
					{
				...
				if (Math.Abs(num6 - 1f) > float.Epsilon)
				{
					...
					stringBuilder.Append("    " + hediffs[num4].LabelBaseCap + ": " + ValueToString(num6, finalized: false, ToStringNumberSense.Factor));
					if (curStage.statFactorEffectMultiplier != null)
					{
			...
			if (ModsConfig.BiotechActive && pawn.genes != null)
			{
				...
				for (int num9 = 0; num9 < genesListForReading.Count; num9++)
				{
					...
					if (statOffsetFromList3 != 0f)
					{
						...
						stringBuilder.AppendLine("    " + genesListForReading[num9].LabelCap + ": " + ValueToString(statOffsetFromList3, finalized: false, ToStringNumberSense.Offset));
					}
					...
					if (statFactorFromList3 != 1f)
					{
						...
						stringBuilder.AppendLine("    " + genesListForReading[num9].LabelCap + ": " + ValueToString(statFactorFromList3, finalized: false, ToStringNumberSense.Factor));
					}
					...
					for (int num10 = 0; num10 < genesListForReading[num9].def.conditionalStatAffecters.Count; num10++)
					{
						...
						if (statOffsetFromList4 != 0f)
						{
							...
							stringBuilder.AppendLine("    " + genesListForReading[num9].LabelCap + " (" + conditionalStatAffecter2.Label + "): " + ValueToString(statOffsetFromList4, finalized: false, ToStringNumberSense.Offset));
						}
						...
						if (statFactorFromList4 != 1f)
						{
							...
							stringBuilder.AppendLine("    " + genesListForReading[num9].LabelCap + " (" + conditionalStatAffecter2.Label + "): " + ValueToString(statFactorFromList4, finalized: false, ToStringNumberSense.Factor));
						}
					}
				}
			}
		...
		if (req.Thing is ThingWithComps { AllComps: var allComps })
		{
			...
			{
				allComps[num11].GetStatsExplanation(stat, stringBuilder);
			}
		}
		if (stat.statFactors != null)
		{
	...
	{
		Pawn pawn2;
		Pawn pawn = (pawn2 = statRequest.Thing as Pawn);
		if (pawn2 == null)
		{
			yield break;
		}
