...
public class SymbolResolver_EdgeDefense : SymbolResolver
{
	...
	public override void Resolve(ResolveParams rp)
	{
		...
		if (num > 0 && ((!rp.settlementDontGeneratePawns) ?? true))
		{
			...
			for (int i = 0; i < num; i++)
			{
				PawnGenerationRequest value = new PawnGenerationRequest(faction.RandomPawnKind(), faction, PawnGenerationContext.NonPlayer, -1, forceGenerateNewPawn: false, allowDead: false, allowDowned: false, canGeneratePawnRelations: true, mustBeCapableOfViolence: true);
