...
{
	private static List<Building> tmpColonyBuildings;

	public static List<ShipLandingArea> tmpShipLandingAreas;
	...
	public static bool TryFindDropSpotNear(IntVec3 center, Map map, out IntVec3 result, bool allowFogged, bool canRoofPunch, bool allowIndoors = true, IntVec2? size = null, bool mustBeReachableFromCenter = true)
	{
		...
		{
			Predicate<IntVec3> v = (IntVec3 c) => validator(c) && c.GetRoom(map) == centerRoom;
			if (TryFindCell(v, out result))
			{
				return true;
			}
		...
		bool TryFindCell(Predicate<IntVec3> validator2, out IntVec3 r)
		{
			int num = 5;
			do
			{
				if (CellFinder.TryFindRandomCellNear(center, map, num, validator2, out r))
				{
					return true;
				}
				num += 3;
	...
	public static bool FindSafeLandingSpot(out IntVec3 spot, Faction faction, Map map, int distToHostiles = 35, int distToFires = 15, int distToEdge = 25, IntVec2? size = null, IntVec3? interactionCell = null)
	{
		...
		if (size.HasValue)
		{
			IntVec2 value = size.Value;
			spot = TryFindSafeLandingSpotCloseToColony(map, value, faction);
			return true;
		}
		int num = 2000;
		while (num-- > 0)
	...
	static DropCellFinder()
	{
		tmpColonyBuildings = new List<Building>();
		tmpShipLandingAreas = new List<ShipLandingArea>();
	}
}
