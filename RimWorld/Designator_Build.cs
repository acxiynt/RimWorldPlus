...
public class Designator_Build : Designator_Place
{
	...
	public override void DrawIcon(Rect rect, Material buttonMat, GizmoRenderParms parms)
	{
		...
		Color? val = (parms.lowLight ? new Color?(Command.LowLightIconColor) : ((Color?)null));
		val = (Color)(((_003F?)val) ?? IconDrawColor);
		Widgets.DefIcon(rect, PlacingDef, StuffDef, 0.85f, thingStyleDef, drawPlaceholder: false, val);
	}
	...
	public override void DrawPanelReadout(ref float curY, float width)
	{
		...
		for (int num = 0; num < tmpcostList.Count; num++)
		{
			...
			float num2 = width - 60f;
			float num3 = Text.CalcHeight(text, num2) - 5f;
			Widgets.Label(new Rect(60f, curY + 2f, num2, num3 + 5f), text);
			curY += num3;
		}
