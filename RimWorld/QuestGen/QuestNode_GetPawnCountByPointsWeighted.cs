...
public class QuestNode_GetPawnCountByPointsWeighted : QuestNode
{
	...
	private void SetVars(Slate slate)
	{
		...
		if (challengeRating.TryGetValue(slate, out var value))
		{
			num = value switch
			{
				1 => Mathf.Min(num, (float)maxCountOneStar.GetValue(slate)), 
				2 => Mathf.Min(num, (float)maxCountTwoStar.GetValue(slate)), 
				_ => Mathf.Min(num, (float)maxCountThreeStar.GetValue(slate)), 
			};
		}
