...
public static class AgeInjuryUtility
{
	...
	public static void GenerateRandomOldAgeInjuries(Pawn pawn, bool tryNotToKillPawn)
	{
		...
		for (int num6 = 1; num6 < pawn.ageTracker.AgeBiologicalYears; num6++)
		{
			foreach (HediffGiver_Birthday item in RandomHediffsToGainOnBirthday(pawn, num6))
			{
				item.TryApplyAndSimulateSeverityChange(pawn, num6, tryNotToKillPawn);
				if (pawn.Dead)
				{
					break;
				}
