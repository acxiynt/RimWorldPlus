...
public static class HealthAIUtility
{
	...
	{
		if (pawn.playerSettings != null && ShouldEverReceiveMedicalCareFromPlayer(pawn))
		{
	...
	public static bool ShouldEverReceiveMedicalCareFromPlayer(Pawn pawn)
	{
		...
		if ((playerSettings == null || playerSettings.medCare != MedicalCareCategory.NoCare) && (pawn.guest == null || !pawn.guest.IsInteractionEnabled(PrisonerInteractionModeDefOf.Execution)))
		{
	...
	{
		if (pawn.Downed && !pawn.InBed() && !pawn.IsCharging() && !CaravanFormingUtility.IsFormingCaravanOrDownedPawnToBeTakenByCaravan(pawn) && (!pawn.IsMutant || pawn.mutant.Def.entitledToMedicalCare) && (!pawn.InMentalState || pawn.health.hediffSet.HasHediff(HediffDefOf.Scaria)) && !pawn.ShouldBeSlaughtered() && (!pawn.TryGetLord(out var lord) || !(lord.LordJob is LordJob_Ritual lordJob_Ritual) || !lordJob_Ritual.TryGetRoleFor(pawn, out var role) || !role.allowDowned))
		{
	...
	{
		if ((forced || patient.Faction == rescuer.Faction) && WantsToBeRescued(patient) && (forced || !patient.IsForbidden(rescuer)) && (forced || !GenAI.EnemyIsNear(patient, 25f)))
		{
			return rescuer.CanReserveAndReach(patient, PathEndMode.OnCell, Danger.Deadly, 1, -1, null, forced);
		}
		return false;
	}
	...
	{
		if (patient.health.hediffSet.BleedRateTotal > 0f && HealthUtility.TicksUntilDeathDueToBloodLoss(patient) < 15000)
		{
			return null;
		}
