...
public class Building_GrowthVat : Building_Enterable, IStoreSettingsParent, IThingHolderWithDrawnPawn, IThingHolder
{
			...
			if (selectedPawn == null)
			{
				...
				command_Action3.action = delegate
				{
					...
					foreach (Pawn item2 in base.Map.mapPawns.AllPawnsSpawned)
					{
						...
						{
							list.Add(new FloatMenuOption(item2.LabelCap, (Action)delegate
							{
								SelectPawn(pawn);
	...
	public override IEnumerable<FloatMenuOption> GetFloatMenuOptions_NonPawn(Thing selectedThing)
	{
		...
		{
			yield return new FloatMenuOption((string)"ImplantEmbryo".Translate(), (Action)delegate
			{
				SelectEmbryo(embryo);
			}, (Thing)embryo, Color.white, MenuOptionPriority.Default, (Action<Rect>)null, (Thing)null, 0f, (Func<Rect, bool>)null, (WorldObject)null, playSelectionSound: true, 0);
		}
		else
		{
