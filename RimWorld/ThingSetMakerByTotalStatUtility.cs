...
public static class ThingSetMakerByTotalStatUtility
{
	...
	public static List<ThingStuffPairWithQuality> GenerateDefsWithPossibleTotalValue(IntRange countRange, float totalValue, IEnumerable<ThingDef> allowed, TechLevel techLevel, QualityGenerator qualityGenerator, Func<ThingStuffPairWithQuality, float> getMinValue, Func<ThingStuffPairWithQuality, float> getMaxValue, Func<ThingStuffPairWithQuality, float> getSingleThingValue, Func<ThingDef, float> weightSelector = null, int tries = 100, float maxMass = float.MaxValue, bool allowNonStackableDuplicates = true, float minSingleItemValue = 0f)
	{
		...
		do
		{
			...
			candidatesTmp.Clear();
			for (int num5 = 0; num5 < allowedThingStuffPairs.Count; num5++)
			{
				ThingStuffPairWithQuality candidate = allowedThingStuffPairs[num5];
			...
			if (countRange.max != int.MaxValue && totalMaxValueSoFar < totalValue * 0.5f)
			{
				...
				{
					ThingStuffPairWithQuality thingStuffPairWithQuality = candidatesTmp[num6];
			...
			for (int num7 = 0; num7 < candidatesTmp.Count; num7++)
			{
				ThingStuffPairWithQuality t = candidatesTmp[num7];
			...
			if (!candidatesTmp.TryRandomElementByWeight(delegate(ThingStuffPairWithQuality x)
			{
				...
				{
					int num9 = countRange.max - chosen.Count;
					float num10 = (totalValue - totalMaxValueSoFar) / (float)num9;
					num8 = Mathf.InverseLerp(0f, num10, MaxValue(x));
				}
				float num11 = 1f;
				if (chosen.Count < countRange.min && totalValue >= totalMinValueSoFar)
				{
					int num12 = countRange.min - chosen.Count;
					float num13 = (totalValue - totalMinValueSoFar) / (float)num12;
					float num14 = Mathf.Max(MinValue(x), trashThreshold);
					if (num14 > num13)
					{
						num11 = num13 / num14;
					}
				}
				float num15 = Mathf.Max(Mathf.Min(num8, num11), 1E-05f);
				if (weightSelector != null)
				{
					num15 *= weightSelector(x.thing);
				}
				if (totalValue > totalMaxValueSoFar)
				{
					int num16 = Mathf.Max(countRange.min - chosen.Count, 1);
					float num17 = Mathf.InverseLerp(0f, maxCandidateMinValue * 0.85f, GetMaxValueWithMaxMass(x, minMassSoFar, maxMass, MinValue, MaxValue) * (float)num16);
					num15 *= num17 * num17;
				}
