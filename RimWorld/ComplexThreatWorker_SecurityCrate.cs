...
public class ComplexThreatWorker_SecurityCrate : ComplexThreatWorker
{
	...
	{
		if (base.CanResolveInt(parms) && ComplexUtility.TryFindRandomSpawnCell(ThingDefOf.AncientSecurityCrate, parms.room.SelectMany((CellRect r) => r.Cells), parms.map, out var _))
		{
	...
	{
		ComplexUtility.TryFindRandomSpawnCell(ThingDefOf.AncientSecurityCrate, parms.room.SelectMany((CellRect r) => r.Cells), parms.map, out var spawnPosition);
