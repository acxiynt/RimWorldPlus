...
public class CompAbilityEffect_ReimplantXenogerm : CompAbilityEffect
{
	...
	public override IEnumerable<Gizmo> CompGetGizmosExtra()
	{
		...
		Command_Action command_Action = new Command_Action
		{
			...
			icon = (Texture)(object)ReimplantIcon.Texture,
			action = delegate
			{
				...
				for (int i = 0; i < list2.Count; i++)
				{
					...
					if (absorber.genes != null && absorber.IsColonistPlayerControlled && !GeneUtility.SameXenotype(absorber, myPawn) && absorber.CanReach(myPawn, PathEndMode.ClosestTouch, Danger.Deadly))
					{
						...
						{
							list.Add(new FloatMenuOption((string)(absorber.LabelCap + ": " + "IdeoligionForbids".Translate()), (Action)null, (Thing)absorber, Color.white, MenuOptionPriority.Default, (Action<Rect>)null, (Thing)null, 0f, (Func<Rect, bool>)null, (WorldObject)null, playSelectionSound: true, 0));
						}
						else
						{
							list.Add(new FloatMenuOption(absorber.LabelShort, (Action)delegate
							{
							...
							}, (Thing)absorber, Color.white, MenuOptionPriority.Default, (Action<Rect>)null, (Thing)null, 0f, (Func<Rect, bool>)null, (WorldObject)null, playSelectionSound: true, 0));
						}
					}
				}
				if (list.Any())
