...
public static class MutantUtility
{
	...
	public static Color? GetSkinColor(Pawn pawn, Color? bodyColor = null)
	{
		...
		Color? val = bodyColor ?? pawn.story?.skinColorOverride ?? pawn.story?.SkinColorBase;
		if (val.HasValue && pawn.mutant.Def == MutantDefOf.Shambler)
		{
			return GetShamblerColor(val.Value);
		}
