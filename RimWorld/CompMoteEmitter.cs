...
public class CompMoteEmitter : ThingComp
{
	...
	public virtual void Emit()
	{
		...
		Vector3 val = Props.offset + Props.RotationOffset(parent.Rotation);
		if (Props.offsetMin != Vector3.zero || Props.offsetMax != Vector3.zero)
		{
			val = Props.EmissionOffset;
		}
		...
		{
			mote = MoteMaker.MakeAttachedOverlay(parent, thingDef, val);
		}
		else
		{
			Vector3 val2 = parent.DrawPos + val;
			if (val2.InBounds(parent.Map))
			{
				mote = MoteMaker.MakeStaticMote(val2, parent.Map, thingDef);
			}
		}
