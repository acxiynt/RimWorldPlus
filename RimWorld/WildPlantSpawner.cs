...
public class WildPlantSpawner : IExposable
{
	...
	private float PlantChoiceWeight(ThingDef plantDef, IntVec3 c, Dictionary<ThingDef, float> distanceSqToNearbyClusters, float wholeMapNumDesiredPlants, float plantDensity)
	{
		...
		{
			float num4 = wholeMapNumDesiredPlants * commonalityPctOfPlant;
			float num5 = (float)Mathf.Max(map.Size.x, map.Size.z) / Mathf.Sqrt(num4) * 2f;
			...
			if (num5 <= 25f)
			{
				num *= LocalPlantProportionsWeightFactor(c, commonalityPctOfPlant, plantDensity, num5, plantDef);
			}
		}
		return num;
	}
	...
	{
		return Mathf.Min(reg.GetBaseDesiredPlantsCount() * plantDensity * map.fertilityGrid.FertilityAt(forCell), (float)reg.CellCount);
	}
}
