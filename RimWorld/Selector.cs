...
public class Selector
{
	...
	private void SelectInsideDragBox()
	{
		...
		Predicate<Thing> predicate = (Thing t) => t.def.category == ThingCategory.Pawn && (((Pawn)t).RaceProps.Humanlike || (ModsConfig.BiotechActive && ((Pawn)t).RaceProps.IsMechanoid)) && t.Faction == Faction.OfPlayer;
		if (SelectWhere(predicate, SelectorUtility.SortInColonistBarOrder))
		{
			return;
		}
		Predicate<Thing> predicate2 = (Thing t) => t.def.category == ThingCategory.Pawn && ((Pawn)t).RaceProps.Humanlike;
		if (SelectWhere(predicate2, null))
		{
			return;
		}
		...
		if (SelectWhere(predicate4, null) || SelectWhere((Thing t) => t.def.selectable, null))
		{
			return;
		}
		...
		bool SelectWhere(Predicate<Thing> predicate5, Action<List<Thing>> postProcessor)
		{
			tmp_selectedBoxThings.Clear();
			foreach (Thing item2 in boxThings.Where((Thing t) => predicate5(t)))
			{
	...
	private void SelectUnderMouse()
	{
		...
		else
		{
			...
			object obj2 = list.Where((object item) => selected.Contains(item)).FirstOrDefault();
			if (obj2 != null)
			{
				if (ShiftIsHeld)
				{
