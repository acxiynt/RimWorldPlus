...
public static class FloatMenuMakerMap
{
					...
					foreach (LocalTargetInfo item9 in GenUI.TargetsAt(clickPos, TargetingParameters.ForDraftedCarryBed(carriedPawn, pawn, GuestStatus.Prisoner), thingsOnly: true))
					{
						...
						else
						{
							...
							item3 = FloatMenuUtility.DecoratePrioritizedTask(new FloatMenuOption(taggedString, delegate
							{
								...
								Job job = JobMaker.MakeJob(JobDefOf.CarryToPrisonerBedDrafted, pawn.carryTracker.CarriedThing, bed);
								job.count = 1;
								pawn.jobs.TryTakeOrderedJob(job, JobTag.Misc);
							}), pawn, bed);
						}
					...
					foreach (LocalTargetInfo item10 in GenUI.TargetsAt(clickPos, TargetingParameters.ForBuilding(), thingsOnly: true))
					{
						...
						else
						{
							...
							item4 = FloatMenuUtility.DecoratePrioritizedTask(new FloatMenuOption(taggedString2, delegate
							{
								...
								Job job = JobMaker.MakeJob(JobDefOf.CarryToEntityHolderAlreadyHolding, thing, pawn.carryTracker.CarriedThing);
								job.count = 1;
								pawn.jobs.TryTakeOrderedJob(job, JobTag.Misc);
							}), pawn, thing);
						}
			...
			if (pawn.health.capacities.CapableOf(PawnCapacityDefOf.Manipulation) && !pawn.IsMutant)
			{
				foreach (LocalTargetInfo item13 in GenUI.TargetsAt(clickPos, TargetingParameters.ForTend(pawn), thingsOnly: true))
				{
					...
					if (medicine != null && action != null && pawn.CanReserve(tendTarget) && tendTarget.Spawned)
					{
						...
						{
							Job job = JobMaker.MakeJob(JobDefOf.TendPatient, tendTarget, null);
							job.count = 1;
							job.draftedTend = true;
							pawn.jobs.TryTakeOrderedJob(job, JobTag.Misc);
						}));
				...
				foreach (LocalTargetInfo item14 in GenUI.TargetsAt(clickPos, TargetingParameters.ForHeldEntity()))
				{
					...
					else if (HealthAIUtility.ShouldBeTendedNowByPlayer(heldPawn) && pawn.CanReserveAndReach(holdingPlatform, PathEndMode.OnCell, Danger.Deadly, 1, -1, null, ignoreOtherReservations: true))
					{
						...
						opts.Add(new FloatMenuOption("Tend".Translate(heldPawn.LabelShort), delegate
						{
							...
							Job job = JobMaker.MakeJob(tendEntity, targetA, (thing2 != null) ? ((LocalTargetInfo)thing2) : LocalTargetInfo.Invalid);
							job.count = 1;
							job.draftedTend = true;
							pawn.jobs.TryTakeOrderedJob(job, JobTag.Misc);
						}));
	...
	private static void AddHumanlikeOrders(Vector3 clickPos, Pawn pawn, List<FloatMenuOption> opts)
	{
		...
		//IL_6251: Unknown result type (might be due to invalid IL or missing references)
		//IL_5cb6: Unknown result type (might be due to invalid IL or missing references)
		//IL_6123: Unknown result type (might be due to invalid IL or missing references)
		...
		//IL_658a: Unknown result type (might be due to invalid IL or missing references)
		//IL_6a2f: Unknown result type (might be due to invalid IL or missing references)
		IntVec3 clickCell = IntVec3.FromVector3(clickPos);
		foreach (Thing thing7 in clickCell.GetThingList(pawn.Map))
		{
		...
		if (pawn.health.capacities.CapableOf(PawnCapacityDefOf.Manipulation))
		{
			foreach (LocalTargetInfo item12 in GenUI.TargetsAt(clickPos, TargetingParameters.ForArrest(pawn), thingsOnly: true))
			{
				...
				Action action = delegate
				{
					Building_Bed building_Bed2 = RestUtility.FindBedFor(pTarg, pawn, checkSocialProperness: false, ignoreOtherReservations: false, GuestStatus.Prisoner);
					if (building_Bed2 == null)
					{
						building_Bed2 = RestUtility.FindBedFor(pTarg, pawn, checkSocialProperness: false, ignoreOtherReservations: true, GuestStatus.Prisoner);
					}
					if (building_Bed2 == null)
					{
					...
					{
						Job job4 = JobMaker.MakeJob(JobDefOf.Arrest, pTarg, building_Bed2);
						job4.count = 1;
				...
				else
				{
					...
					opt = FloatMenuUtility.DecoratePrioritizedTask(new FloatMenuOption(text, delegate
					{
						...
						if (maxAmountToPickup2 != 0)
						{
							...
							Job job4 = JobMaker.MakeJob(JobDefOf.Ingest, t);
							job4.count = maxAmountToPickup2;
							pawn.jobs.TryTakeOrderedJob(job4, JobTag.Misc);
						}
					}, priority), pawn, t);
		...
		if (pawn.health.capacities.CapableOf(PawnCapacityDefOf.Manipulation))
		{
			...
			foreach (Thing item15 in thingList)
			{
				...
				if (ModsConfig.BiotechActive && corpse2.InnerPawn.health.hediffSet.HasHediff(HediffDefOf.MechlinkImplant))
				{
					...
					{
						Job job4 = JobMaker.MakeJob(JobDefOf.RemoveMechlink, corpse2);
						pawn.jobs.TryTakeOrderedJob(job4, JobTag.Misc);
					}), pawn, new LocalTargetInfo(corpse2)));
				}
			}
			foreach (LocalTargetInfo item16 in GenUI.TargetsAt(clickPos, TargetingParameters.ForRescue(pawn), thingsOnly: true))
			{
				...
				if (!victim.IsPrisonerOfColony && !victim.IsSlaveOfColony && !victim.IsColonyMech)
				{
					...
					if (victim.Faction == Faction.OfPlayer || victim.Faction == null || !victim.Faction.HostileTo(Faction.OfPlayer) || isBaby)
					{
						FloatMenuOption floatMenuOption = FloatMenuUtility.DecoratePrioritizedTask(new FloatMenuOption((HealthAIUtility.ShouldSeekMedicalRest(victim) || !victim.ageTracker.CurLifeStage.alwaysDowned) ? "Rescue".Translate(victim.LabelCap, victim) : "PutSomewhereSafe".Translate(victim.LabelCap, victim), delegate
						{
							...
							{
								Building_Bed building_Bed2 = RestUtility.FindBedFor(victim, pawn, checkSocialProperness: false);
								if (building_Bed2 == null)
								{
									building_Bed2 = RestUtility.FindBedFor(victim, pawn, checkSocialProperness: false, ignoreOtherReservations: true);
								}
								if (building_Bed2 == null)
								{
									string text7 = ((!victim.RaceProps.Animal) ? ((string)"NoNonPrisonerBed".Translate()) : ((string)"NoAnimalBed".Translate()));
									Messages.Message("CannotRescue".Translate() + ": " + text7, victim, MessageTypeDefOf.RejectInput, historical: false);
								}
								else
								{
				...
				{
					Building_Bed building_Bed2 = RestUtility.FindBedFor(victim, pawn, checkSocialProperness: false, ignoreOtherReservations: false, GuestStatus.Prisoner);
					if (building_Bed2 == null)
					{
						building_Bed2 = RestUtility.FindBedFor(victim, pawn, checkSocialProperness: false, ignoreOtherReservations: true, GuestStatus.Prisoner);
					}
					if (building_Bed2 == null)
					{
					...
					{
						Job job4 = JobMaker.MakeJob(JobDefOf.Capture, victim, building_Bed2);
						job4.count = 1;
						pawn.jobs.TryTakeOrderedJob(job4, JobTag.Misc);
			...
			foreach (LocalTargetInfo item17 in GenUI.TargetsAt(clickPos, TargetingParameters.ForRescue(pawn), thingsOnly: true))
			{
				...
				Action action2 = delegate
				{
					...
					{
						Job job4 = JobMaker.MakeJob(jDef, victim2, building_CryptosleepCasket);
						job4.count = 1;
						pawn.jobs.TryTakeOrderedJob(job4, JobTag.Misc);
					}
				};
			...
			if (ModsConfig.AnomalyActive && pawn.ageTracker.AgeBiologicalYears >= 10)
			{
				foreach (LocalTargetInfo item18 in GenUI.TargetsAt(clickPos, TargetingParameters.ForEntityCapture(), thingsOnly: true))
				{
					...
					IEnumerable<Building_HoldingPlatform> enumerable2 = from x in pawn.Map.listerBuildings.AllBuildingsColonistOfClass<Building_HoldingPlatform>()
						...
						select x;
					Thing building = GenClosest.ClosestThing_Global_Reachable(pawn.Position, pawn.Map, enumerable2, PathEndMode.ClosestTouch, TraverseParms.For(pawn, Danger.Some), 9999f, null, delegate(Thing thing7)
					{
						CompEntityHolder compEntityHolder = thing7.TryGetComp<CompEntityHolder>();
						return (compEntityHolder != null && compEntityHolder.ContainmentStrength >= studyTarget.GetStatValue(StatDefOf.MinimumContainmentStrength)) ? (compEntityHolder.ContainmentStrength / Mathf.Max(studyTarget.PositionHeld.DistanceTo(thing7.Position), 1f)) : 0f;
					});
					if (building != null)
					{
						opts.Add(FloatMenuUtility.DecoratePrioritizedTask(new FloatMenuOption("Capture".Translate(studyTarget.Label, studyTarget), delegate
						{
							...
							Job job4 = JobMaker.MakeJob(JobDefOf.CarryToEntityHolder, building, studyTarget);
							job4.count = 1;
							pawn.jobs.TryTakeOrderedJob(job4, JobTag.Misc);
						}), pawn, studyTarget));
				...
				foreach (LocalTargetInfo item19 in GenUI.TargetsAt(clickPos, TargetingParameters.ForHeldEntity(), thingsOnly: true))
				{
					...
					}, delegate(Thing thing7)
					{
						CompEntityHolder compEntityHolder = thing7.TryGetComp<CompEntityHolder>();
						return (compEntityHolder != null && compEntityHolder.ContainmentStrength >= heldPawn.GetStatValue(StatDefOf.MinimumContainmentStrength)) ? (compEntityHolder.ContainmentStrength / Mathf.Max(heldPawn.PositionHeld.DistanceTo(thing7.Position), 1f)) : 0f;
					}) != null)
					{
			...
			if (ModsConfig.RoyaltyActive)
			{
				foreach (LocalTargetInfo item21 in GenUI.TargetsAt(clickPos, TargetingParameters.ForShuttle(pawn), thingsOnly: true))
				{
					...
					void CarryToShuttleAct()
					{
						...
						Job job4 = JobMaker.MakeJob(JobDefOf.HaulToTransporter, victim3, shuttleThing);
						job4.ignoreForbidden = true;
						job4.count = 1;
						pawn.jobs.TryTakeOrderedJob(job4, JobTag.Misc);
					}
					bool IsValidShuttle(Thing thing7)
					{
						return thing7.TryGetComp<CompShuttle>()?.IsAllowedNow(victim3) ?? false;
					}
				}
			}
			if (ModsConfig.IdeologyActive)
			{
				foreach (Thing thing in thingList)
				{
					...
					{
						opts.Add(new FloatMenuOption("CannotHack".Translate(thing.Label) + ": " + "AlreadyHacked".Translate(), null));
					}
					else if (!HackUtility.IsCapableOfHacking(pawn))
					{
						opts.Add(new FloatMenuOption("CannotHack".Translate(thing.Label) + ": " + "IncapableOfHacking".Translate(), null));
					}
					else if (!pawn.CanReach(thing, PathEndMode.ClosestTouch, Danger.Deadly))
					{
						opts.Add(new FloatMenuOption("CannotHack".Translate(thing.Label) + ": " + "NoPath".Translate().CapitalizeFirst(), null));
					}
					else if (thing.def == ThingDefOf.AncientEnemyTerminal)
					{
						opts.Add(FloatMenuUtility.DecoratePrioritizedTask(new FloatMenuOption("Hack".Translate(thing.Label), delegate
						{
							...
							{
								pawn.jobs.TryTakeOrderedJob(JobMaker.MakeJob(JobDefOf.Hack, thing), JobTag.Misc);
							}));
				...
				foreach (LocalTargetInfo thing2 in GenUI.TargetsAt(clickPos, TargetingParameters.ForBuilding(ThingDefOf.ArchonexusCore)))
				{
					if (!pawn.CanReach(thing2, PathEndMode.InteractionCell, Danger.Deadly))
					{
						opts.Add(new FloatMenuOption("CannotInvoke".Translate("Power".Translate()) + ": " + "NoPath".Translate().CapitalizeFirst(), null));
						continue;
					}
					if (!((Building_ArchonexusCore)(Thing)thing2).CanActivateNow)
					{
					...
					{
						pawn.jobs.TryTakeOrderedJob(JobMaker.MakeJob(JobDefOf.ActivateArchonexusCore, thing2), JobTag.Misc);
					}), pawn, thing2));
				}
			}
			if (ModsConfig.IdeologyActive)
			{
				foreach (Thing thing3 in thingList)
				{
					...
					if (container.Full)
					{
						...
						{
							Job job = JobMaker.MakeJob(JobDefOf.ExtractRelic, thing3, container.ContainedThing, foundCell);
							job.count = 1;
					...
					else
					{
						...
						else
						{
							...
							{
								Job job2 = JobMaker.MakeJob(JobDefOf.InstallRelic, item22, thing3, thing3.InteractionCell);
								job2.count = 1;
					...
					if (!pawn.Map.IsPlayerHome && !pawn.IsFormingCaravan() && container.Full)
					{
						...
						{
							Job job4 = JobMaker.MakeJob(JobDefOf.ExtractToInventory, thing3, container.ContainedThing, thing3.InteractionCell);
							job4.count = 1;
							pawn.jobs.TryTakeOrderedJob(job4, JobTag.Misc);
						}), pawn, new LocalTargetInfo(thing3)));
					}
				...
				foreach (Thing item23 in thingList)
				{
					...
					IEnumerable<Thing> searchSet = from x in item23.Map.listerThings.ThingsOfDef(ThingDefOf.Reliquary)
						...
						select x;
					Thing thing4 = GenClosest.ClosestThing_Global_Reachable(item23.Position, item23.Map, searchSet, PathEndMode.Touch, TraverseParms.For(pawn), 9999f, (Thing thing7) => pawn.CanReserve(thing7));
					if (thing4 == null)
					{
						opts.Add(new FloatMenuOption("InstallInReliquary".Translate() + " (" + "NoEmptyReliquary".Translate() + ")", null));
						continue;
					}
					Job job3 = JobMaker.MakeJob(JobDefOf.InstallRelic, item23, thing4, thing4.InteractionCell);
					job3.count = 1;
			...
			if (ModsConfig.BiotechActive && MechanitorUtility.IsMechanitor(pawn))
			{
				...
				{
					Pawn mech;
					if ((mech = thing5 as Pawn) == null || !mech.IsColonyMech)
					{
						continue;
					}
					if (mech.GetOverseer() != pawn)
					{
						...
						else
						{
							...
							{
								Job job4 = JobMaker.MakeJob(JobDefOf.ControlMech, thing5);
								pawn.jobs.TryTakeOrderedJob(job4, JobTag.Misc);
					...
					else
					{
						...
						}, MenuOptionPriority.Low, null, null, 0f, null, null, playSelectionSound: true, -10), pawn, new LocalTargetInfo(thing5)));
						if (!mech.IsFighting())
						{
							opts.Add(FloatMenuUtility.DecoratePrioritizedTask(new FloatMenuOption("DisassembleMech".Translate(mech.LabelCap), delegate
							{
								...
								{
									pawn.jobs.TryTakeOrderedJob(JobMaker.MakeJob(JobDefOf.DisassembleMech, thing5), JobTag.Misc);
								}, destructive: true));
							}, MenuOptionPriority.Low, null, null, 0f, null, null, playSelectionSound: true, -20), pawn, new LocalTargetInfo(thing5)));
						}
					}
					...
					{
						Job job4 = JobMaker.MakeJob(JobDefOf.RepairMech, mech);
						pawn.jobs.TryTakeOrderedJob(job4, JobTag.Misc);
					}), pawn, new LocalTargetInfo(thing5)));
				}
			}
			if (ModsConfig.BiotechActive)
			{
				foreach (Thing item24 in thingList)
				{
					...
					{
						Job job4 = JobMaker.MakeJob(JobDefOf.HaulMechToCharger, p, charger, charger.InteractionCell);
						job4.count = 1;
						pawn.jobs.TryTakeOrderedJob(job4, JobTag.Misc);
		...
		if (ModsConfig.BiotechActive && pawn.CanDeathrest())
		{
			...
			for (int num = 0; num < thingList2.Count; num++)
			{
				...
				else if (RestUtility.IsValidBedFor(bed, pawn, pawn, checkSocialProperness: true, allowMedBedEvenIfSetToNoCare: false, ignoreOtherReservations: false, pawn.GuestStatus))
				{
					...
					{
						Job job4 = JobMaker.MakeJob(JobDefOf.Deathrest, bed);
						job4.forceSleep = true;
						pawn.jobs.TryTakeOrderedJob(job4, JobTag.Misc);
					}));
				}
			}
		...
		if (ModsConfig.BiotechActive && pawn.IsBloodfeeder() && pawn.genes?.GetFirstGeneOfType<Gene_Hemogen>() != null)
		{
			foreach (LocalTargetInfo item26 in GenUI.TargetsAt(clickPos, TargetingParameters.ForBloodfeeding(pawn)))
			{
				...
				if (acceptanceReport3.Accepted)
				{
					...
					{
						Job job4 = JobMaker.MakeJob(JobDefOf.PrisonerBloodfeed, targPawn);
						pawn.jobs.TryTakeOrderedJob(job4, JobTag.Misc);
					}), pawn, targPawn));
				}
		...
		if (ModsConfig.BiotechActive && pawn.health.capacities.CapableOf(PawnCapacityDefOf.Manipulation))
		{
			foreach (LocalTargetInfo item27 in GenUI.TargetsAt(clickPos, TargetingParameters.ForCarryDeathresterToBed(pawn)))
			{
				...
				if (bestBedOrCasket != null)
				{
					...
					{
						Job job4 = JobMaker.MakeJob(JobDefOf.DeliverToBed, targPawn2, bestBedOrCasket);
						job4.count = 1;
						pawn.jobs.TryTakeOrderedJob(job4, JobTag.Misc);
		...
		if (ModsConfig.BiotechActive && !pawn.Downed && !pawn.Drafted)
		{
			foreach (LocalTargetInfo item29 in GenUI.TargetsAt(clickPos, TargetingParameters.ForBabyCare(pawn), thingsOnly: true))
			{
				...
				if (safePlace.Thing is Building_Bed building_Bed)
				{
				...
				{
					Job job4 = JobMaker.MakeJob(JobDefOf.BringBabyToSafety, baby, safePlace);
					job4.count = 1;
					pawn.jobs.TryTakeOrderedJob(job4, JobTag.Misc);
				}), pawn, baby);
		...
		if (pawn.equipment != null)
		{
			...
			for (int num2 = 0; num2 < thingList3.Count; num2++)
			{
				if (thingList3[num2].TryGetComp<CompEquippable>() == null)
				{
					continue;
				}
				ThingWithComps equipment = (ThingWithComps)thingList3[num2];
		...
		foreach (Pair<IReloadableComp, Thing> item32 in ReloadableUtility.FindPotentiallyReloadableGear(pawn, clickCell.GetThingList(pawn.Map)))
		{
			...
			string text5 = "Reload".Translate(thingComp.parent.Named("GEAR"), NamedArgumentUtility.Named(reloadable.AmmoDef, "AMMO")) + " (" + reloadable.LabelRemaining + ")";
			...
			{
				opts.Add(new FloatMenuOption(text5 + ": " + "ReloadCannotCarryEnough".Translate(NamedArgumentUtility.Named(reloadable.AmmoDef, "AMMO")), null));
				continue;
			}
			...
			opts.Add(FloatMenuUtility.DecoratePrioritizedTask(new FloatMenuOption(text5, action3), pawn, second));
		}
			...
			foreach (Thing item33 in pawn.Map.thingGrid.ThingsAt(clickCell))
			{
				...
				FloatMenuOption item6 = ((!pawn.CanReach(apparel, PathEndMode.ClosestTouch, Danger.Deadly)) ? new FloatMenuOption(key2.Translate(apparel.Label, apparel) + ": " + "NoPath".Translate().CapitalizeFirst(), null) : (apparel.IsBurning() ? new FloatMenuOption(key2.Translate(apparel.Label, apparel) + ": " + "Burning".Translate(), null) : (pawn.apparel.WouldReplaceLockedApparel(apparel) ? new FloatMenuOption(key2.Translate(apparel.Label, apparel) + ": " + "WouldReplaceLockedApparel".Translate().CapitalizeFirst(), null) : ((!ApparelUtility.HasPartsToWear(pawn, apparel.def)) ? new FloatMenuOption(key2.Translate(apparel.Label, apparel) + ": " + "CannotWearBecauseOfMissingBodyParts".Translate().CapitalizeFirst(), null) : (EquipmentUtility.CanEquip(apparel, pawn, out cantReason2) ? FloatMenuUtility.DecoratePrioritizedTask(new FloatMenuOption(key3.Translate(apparel.LabelShort, apparel), delegate
				{
					Action action7 = delegate
					{
						...
						Job job4 = JobMaker.MakeJob(JobDefOf.Wear, apparel);
						pawn.jobs.TryTakeOrderedJob(job4, JobTag.Misc);
					};
			...
			foreach (Thing item7 in clickCell.GetItems(pawn.Map))
			{
				...
				if (item7.stackCount == 1)
				{
					...
					opts.Add(FloatMenuUtility.DecoratePrioritizedTask(new FloatMenuOption(CaravanFormingUtility.AppendOverweightInfo("LoadIntoCaravan".Translate(item7.Label, item7), capacityLeft2), delegate
					{
						...
						Job job4 = JobMaker.MakeJob(jobDef, item7);
						job4.count = 1;
						job4.checkEncumbrance = packTarget == pawn;
						pawn.jobs.TryTakeOrderedJob(job4, JobTag.Misc);
			...
			foreach (Thing item8 in clickCell.GetItems(pawn.Map))
			{
				...
				if (item8.stackCount == 1 || maxAllowedToPickUp == 1)
				{
					opts.Add(FloatMenuUtility.DecoratePrioritizedTask(new FloatMenuOption("PickUpOne".Translate(item8.LabelNoCount, item8), delegate
					{
						...
						Job job4 = JobMaker.MakeJob(JobDefOf.TakeInventory, item8);
						job4.count = 1;
						job4.checkEncumbrance = true;
						job4.takeInventoryDelay = 120;
						pawn.jobs.TryTakeOrderedJob(job4, JobTag.Misc);
				...
				{
					int num3 = Mathf.Min(MassUtility.CountToPickUpUntilOverEncumbered(pawn, item8), item8.stackCount);
					int to = Mathf.Min(maxAllowedToPickUp, num3);
					Dialog_Slider window = new Dialog_Slider("PickUpCount".Translate(item8.LabelNoCount, item8), 1, to, delegate(int count)
					{
						item8.SetForbidden(value: false, warnOnFail: false);
						Job job4 = JobMaker.MakeJob(JobDefOf.TakeInventory, item8);
						job4.count = count;
			...
			foreach (Thing item9 in clickCell.GetItems(pawn.Map))
			{
				...
				if (item9.stackCount == 1)
				{
					opts.Add(FloatMenuUtility.DecoratePrioritizedTask(new FloatMenuOption("GiveToPackAnimal".Translate(item9.Label, item9), delegate
					{
						...
						Job job4 = JobMaker.MakeJob(JobDefOf.GiveToPackAnimal, item9);
						job4.count = 1;
						pawn.jobs.TryTakeOrderedJob(job4, JobTag.Misc);
				...
				{
					int to = Mathf.Min(MassUtility.CountToPickUpUntilOverEncumbered(bestPackAnimal, item9), item9.stackCount);
					Dialog_Slider window = new Dialog_Slider("GiveToPackAnimalCount".Translate(item9.LabelNoCount, item9), 1, to, delegate(int count)
					{
						item9.SetForbidden(value: false, warnOnFail: false);
						Job job4 = JobMaker.MakeJob(JobDefOf.GiveToPackAnimal, item9);
						job4.count = count;
		...
		if (!pawn.Map.IsPlayerHome && pawn.Map.exitMapGrid.MapUsesExitGrid)
		{
			foreach (LocalTargetInfo item34 in GenUI.TargetsAt(clickPos, TargetingParameters.ForRescue(pawn), thingsOnly: true))
			{
				...
				else if (pawn.Map.IsPocketMap)
				{
					...
					opts.Add(FloatMenuUtility.DecoratePrioritizedTask(new FloatMenuOption(taggedString3, delegate
					{
						Job job4 = JobMaker.MakeJob(JobDefOf.CarryDownedPawnToPortal, portal, p2);
						job4.count = 1;
						pawn.jobs.TryTakeOrderedJob(job4, JobTag.Misc);
				...
				else
				{
					...
					{
						Job job4 = JobMaker.MakeJob(JobDefOf.CarryDownedPawnToExit, p2, exitSpot);
						job4.count = 1;
						job4.failIfCantJoinOrCreateCaravan = true;
						pawn.jobs.TryTakeOrderedJob(job4, JobTag.Misc);
					}, MenuOptionPriority.High), pawn, item34));
				}
			}
		...
		foreach (LocalTargetInfo item35 in GenUI.TargetsAt(clickPos, TargetingParameters.ForTrade(), thingsOnly: true))
		{
			...
			else
			{
				...
				{
					Job job4 = JobMaker.MakeJob(JobDefOf.TradeWithPawn, pTarg2);
				...
				{
					text6 = " (" + pTarg2.Faction.Name + ")";
				}
				opts.Add(FloatMenuUtility.DecoratePrioritizedTask(new FloatMenuOption("TradeWith".Translate(pTarg2.LabelShort + ", " + pTarg2.TraderKind.label) + text6, action5, MenuOptionPriority.InitiateSocial, null, dest.Thing), pawn, pTarg2));
			}
			if (pTarg2.GetLord().LordJob is LordJob_TradeWithColony && !pTarg2.mindState.traderDismissed)
			{
				Action action6 = delegate
				{
					Job job4 = JobMaker.MakeJob(JobDefOf.DismissTrader, pTarg2);
					job4.playerForced = true;
					pawn.jobs.TryTakeOrderedJob(job4, JobTag.Misc);
				};
				opts.Add(FloatMenuUtility.DecoratePrioritizedTask(new FloatMenuOption("DismissTrader".Translate(), action6, MenuOptionPriority.InitiateSocial, null, dest.Thing), pawn, pTarg2));
			}
		}
		foreach (LocalTargetInfo casket in GenUI.TargetsAt(clickPos, TargetingParameters.ForOpen(pawn), thingsOnly: true))
		{
			...
			{
				opts.Add(new FloatMenuOption("CannotOpen".Translate(casket.Thing) + ": " + "Incapable".Translate().CapitalizeFirst(), null));
			}
		...
		if (pawn.health.capacities.CapableOf(PawnCapacityDefOf.Manipulation) && new TargetInfo(clickCell, pawn.Map).IsBurning())
		{
			...
			else
			{
				...
				{
					Job job4 = JobMaker.MakeJob(JobDefOf.ExtinguishFiresNearby);
					foreach (Fire item36 in clickCell.GetFiresNearCell(pawn.Map))
					{
						job4.AddQueuedTarget(TargetIndex.A, item36);
					}
					pawn.jobs.TryTakeOrderedJob(job4, JobTag.Misc);
				});
			}
		...
		if (!pawn.Drafted && pawn.health.capacities.CapableOf(PawnCapacityDefOf.Manipulation) && !pawn.WorkTypeIsDisabled(WorkTypeDefOf.Cleaning))
		{
			...
			if (room != null && room.ProperRoom && !room.PsychologicallyOutdoors && !room.TouchesMapEdge)
			{
				...
				if (!filth.EnumerableNullOrEmpty())
				{
					...
					{
						Job job4 = JobMaker.MakeJob(JobDefOf.Clean);
						foreach (Filth item37 in filth)
						{
							job4.AddQueuedTarget(TargetIndex.A, item37);
						}
						pawn.jobs.TryTakeOrderedJob(job4, JobTag.Misc);
					}));
				}
		...
		foreach (Thing item38 in pawn.Map.thingGrid.ThingsAt(clickCell))
		{
			...
			if ((compSelectProxy = thing6.TryGetComp<CompSelectProxy>()) != null && compSelectProxy.thingToSelect != null)
			{
				thing6 = compSelectProxy.thingToSelect;
			}
			foreach (FloatMenuOption floatMenuOption4 in thing6.GetFloatMenuOptions(pawn))
			{
	...
	private static void AddJobGiverWorkOrders(Vector3 clickPos, Pawn pawn, List<FloatMenuOption> opts, bool drafted)
	{
		...
		foreach (Thing thing2 in clickCell.GetThingList(pawn.Map))
		{
			...
			for (int num = 0; num < equivalenceGroupTempStorage.Length; num++)
			{
				if (equivalenceGroupTempStorage[num] != null)
				{
					opts.Add(equivalenceGroupTempStorage[num]);
					equivalenceGroupTempStorage[num] = null;
				}
			}
		}
		foreach (WorkTypeDef item2 in DefDatabase<WorkTypeDef>.AllDefsListForReading)
		{
			for (int num2 = 0; num2 < item2.workGiversByPriority.Count; num2++)
			{
				WorkGiverDef workGiver2 = item2.workGiversByPriority[num2];
