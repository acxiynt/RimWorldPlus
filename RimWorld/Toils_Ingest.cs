...
public class Toils_Ingest
{
	...
	public static Toil ReserveFoodFromStackForIngesting(TargetIndex ind, Pawn ingester = null)
	{
		...
		toil.initAction = delegate
		{
			...
			{
				int num = FoodUtility.WillIngestStackCountOf(ingester, target.Thing.def, FoodUtility.NutritionForEater(ingester, target.Thing));
				stackCount = Mathf.Min(target.Thing.stackCount, num);
			}
