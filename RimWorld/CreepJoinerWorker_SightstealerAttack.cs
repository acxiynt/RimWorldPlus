...
public class CreepJoinerWorker_SightstealerAttack : BaseCreepJoinerWorker
{
	...
	public override void DoResponse(List<TargetInfo> looktargets, List<NamedArgument> namedArgs)
	{
		...
		float num = StorytellerUtility.DefaultThreatPointsNow(base.Pawn.MapHeld) * SpawnPointScale.RandomInRange;
		num = Mathf.Max(Faction.OfEntities.def.MinPointsToGeneratePawnGroup(PawnGroupKindDefOf.Sightstealers), num);
