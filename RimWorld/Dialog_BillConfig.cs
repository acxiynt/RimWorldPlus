	...
	public override void DoWindowContents(Rect inRect)
	{
		...
		if (bill is Bill_Mech bill_Mech)
		{
			...
			if (!bill.recipe.mechResurrection)
			{
				...
				stringBuilder.AppendLine((string)(Find.ActiveLanguageWorker.Pluralize(ThingDefOf.Wastepack.LabelCap) + " " + "ThingsProduced".Translate() + ": ") + num2);
			}
		}
				...
				if (listing_Standard5.ButtonText(text7))
				{
					...
					foreach (StyleCategoryDef item3 in relevantStyleCategories)
					{
						foreach (ThingDefStyle style in item3.thingDefStyles)
						{
							...
							{
								for (int num3 = 0; num3 < graphic_Random.SubGraphicsCount; num3++)
								{
									int index = num3;
		...
		if (bill.recipe.products.Count == 1)
		{
			...
			Widgets.InfoCardButton(((Rect)(ref rect)).x, ((Rect)(ref val)).y, thingDef3, GenStuff.DefaultStuffFor(thingDef3));
		}
	}
