...
public static class ReloadableUtility
{
	...
	public static IEnumerable<Pair<IReloadableComp, Thing>> FindPotentiallyReloadableGear(Pawn pawn, List<Thing> potentialAmmo)
	{
		if (pawn.equipment?.PrimaryEq != null)
		{
			...
			if (TryGetAmmo(reloadableComp, out var ammo))
			{
				yield return new Pair<IReloadableComp, Thing>(reloadableComp, ammo);
			}
		}
		if (pawn.apparel == null)
		{
		...
		foreach (Apparel item in pawn.apparel.WornApparel)
		{
			...
			if (TryGetAmmo(reloadableComp2, out var ammo2))
			{
				yield return new Pair<IReloadableComp, Thing>(reloadableComp2, ammo2);
			}
		}
