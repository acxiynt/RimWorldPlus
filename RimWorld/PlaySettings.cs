...
public sealed class PlaySettings : IExposable
{
	...
	public void DoPlaySettingsGlobalControls(WidgetRow row, bool worldView)
	{
		...
		else
		{
			...
			string tooltip = string.Format(arg1: KeyPrefs.KeyPrefsData.GetBoundKeyCode(KeyBindingDefOf.OpenMapSearch, KeyPrefs.BindingSlot.A).ToStringReadable(), format: "{0}: {1}\n\n{2}", arg0: "HotKeyTip".Translate(), arg2: "SearchTheMapDesc".Translate());
			if (Find.CurrentMap != null && (row.ButtonIcon(TexButton.SearchButton, tooltip) || KeyBindingDefOf.OpenMapSearch.JustPressed))
			{
