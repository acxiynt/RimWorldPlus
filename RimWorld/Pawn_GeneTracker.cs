...
public class Pawn_GeneTracker : IExposable
{
	...
	private void Notify_GenesChanged(GeneDef addedOrRemovedGene)
	{
		...
		{
			Color val = chosen2.def.hairColorOverride.Value;
			if (chosen2.def.randomBrightnessFactor != 0f)
			{
				val *= 1f + Rand.Range(0f - chosen2.def.randomBrightnessFactor, chosen2.def.randomBrightnessFactor);
			}
		...
		if (ModLister.BiotechInstalled)
		{
			if (addedOrRemovedGene.skinColorOverride.HasValue)
			{
				...
				{
					Color val2 = chosen4.def.skinColorOverride.Value;
					if (chosen4.def.randomBrightnessFactor != 0f)
					{
						val2 *= 1f + Rand.Range(0f - chosen4.def.randomBrightnessFactor, chosen4.def.randomBrightnessFactor);
					}
