...
public class WorldGenStep_AncientRoads : WorldGenStep
{
	...
	private void GenerateAncientRoads()
	{
		...
		{
			if (list[num].Any((int elem) => used.Contains(elem)))
			{
				continue;
			}
			if (list[num].Count < 4)
			...
			for (int num2 = 0; num2 < list[num].Count - 1; num2++)
			{
				...
				using WorldPath worldPath = Find.WorldPathFinder.FindPath(list[num][num2], list[num][num2 + 1], null, (float cost) => cost > costCutoff);
