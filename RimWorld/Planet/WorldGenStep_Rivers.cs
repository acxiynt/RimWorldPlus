...
public class WorldGenStep_Rivers : WorldGenStep
{
	...
	private void GenerateRivers()
	{
		...
		List<int>[] array = Find.WorldPathFinder.FloodPathsWithCostForTree(coastalWaterTiles, delegate(int st, int ed)
		{
			...
			int num2 = neighbors[0];
			for (int i = 0; i < neighbors.Count; i++)
			{
				if (GetImpliedElevation(Find.WorldGrid[neighbors[i]]) < GetImpliedElevation(Find.WorldGrid[num2]))
				{
					num2 = neighbors[i];
				}
			}
			...
			return Mathf.RoundToInt(num3 * ElevationChangeCost.Evaluate(GetImpliedElevation(tile2) - GetImpliedElevation(tile)));
	...
	private void ExtendRiver(float[] flow, List<int>[] riverPaths, int index, RiverDef incomingRiver)
	{
		...
		{
			RiverDef.Branch branch = incomingRiver.branches.Where((RiverDef.Branch branch2) => (float)branch2.minFlow <= flow[alternateRiver]).MaxByWithFallback((RiverDef.Branch branch2) => branch2.minFlow);
			if (branch != null && Rand.Value < branch.chance)
			{
				Find.WorldGrid.OverlayRiver(index, alternateRiver, branch.child);
