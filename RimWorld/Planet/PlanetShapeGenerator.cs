...
public static class PlanetShapeGenerator
{
	...
	private static void Subdivide(bool lastPass)
	{
		...
		for (int count2 = tris.Count; i < count2; i++)
		{
			...
			Vector3 val = (verts[triangleIndices.v1] + verts[triangleIndices.v2] + verts[triangleIndices.v3]) / 3f;
	...
	private static bool ShouldDiscardGeneratedTile(List<int> generatedTileVerts)
	{
		...
		{
			val += verts[generatedTileVerts[i]];
		}
		return !MeshUtility.VisibleForWorldgen(val / (float)generatedTileVerts.Count, radius, viewCenter, viewAngle);
	}
