...
public static class CaravanPawnsNeedsUtility
{
	...
	public static float GetFoodScore(Thing food, Pawn pawn)
	{
		...
		{
			int num2 = food.TryGetComp<CompRottable>()?.TicksUntilRotAtCurrentTemp ?? int.MaxValue;
			float num3 = 1f - (float)Mathf.Min(num2, 3600000) / 3600000f;
			num += Mathf.Min(num3, 0.999f);
		}
		return num;
	}
