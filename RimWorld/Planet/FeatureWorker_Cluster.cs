	...
	private void CalculateContiguousGroups()
	{
		...
		for (int i = 0; i < roots.Count; i++)
		{
			...
			for (int num2 = 0; num2 < tmpGroup.Count; num2++)
			{
				...
				if (anyMember)
				{
					FeatureWorker.groupID[tmpGroup[num2]] = i + 1;
				}
			}
		}
		FeatureWorker.ClearVisited();
		...
		{
			int num4 = roots[num3];
			if (FeatureWorker.visited[num4] || FeatureWorker.groupSize[num4] < minRootGroupSize || FeatureWorker.groupSize[num4] > maxRootGroupSize || FeatureWorker.groupSize[num4] > maxOverallSize)
			{
				continue;
			}
			currentGroup.Clear();
			visitedValidGroupIDs.Clear();
			worldFloodFiller.FloodFill(num4, delegate(int x)
			{
			...
			{
				int num6 = currentGroup[num5];
				if (IsMember(num6, out var ifRootThenRootGroupSizeMustMatch) && (!ifRootThenRootGroupSizeMustMatch || !rootsSet.Contains(num6) || (FeatureWorker.groupSize[num6] >= minRootGroupSize && FeatureWorker.groupSize[num6] <= maxRootGroupSize)))
				{
