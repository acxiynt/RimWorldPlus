...
public class WorldLayer_Hills : WorldLayer
{
	...
	public override IEnumerable Regenerate()
	{
		...
		for (int i = 0; i < tilesCount; i++)
		{
			...
			Vector3 val = worldGrid.GetTileCenter(i);
			Vector3 posForTangents = val;
			float magnitude = ((Vector3)(ref val)).magnitude;
			val += Rand.UnitVector3 * floatRange.RandomInRange * worldGrid.averageTileSize;
			val = ((Vector3)(ref val)).normalized * magnitude;
			WorldRendererUtility.PrintQuadTangentialToPlanet(val, posForTangents, BaseSizeRange.RandomInRange * worldGrid.averageTileSize, 0.005f, subMesh, counterClockwise: false, randomizeRotation: true, printUVs: false);
