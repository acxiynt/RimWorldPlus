...
public static class ITab_Pawn_Log_Utility
{
	...
	public class LogLineDisplayableLog : LogLineDisplayable
	{
		...
		public override void Draw(float position, float width, LogDrawData data)
		{
			...
			((Rect)(ref val))._002Ector(0f, position, width, height);
			if (log == data.highlightEntry)
			{
				Widgets.DrawRectFast(val, new Color(1f, 1f, 1f, HighlightAlpha * data.highlightIntensity));
			...
			{
				Widgets.DrawRectFast(val, new Color(1f, 1f, 1f, AlternateAlpha));
			}
			data.alternatingBackground = !data.alternatingBackground;
			TaggedString label = log.ToGameStringFromPOV(pawn);
			Widgets.Label(new Rect(29f, position, num, height), label);
			Texture2D val2 = log.IconFromPOV(pawn);
			if ((Object)(object)val2 != (Object)null)
			{
				GUI.color = (Color)(((_003F?)log.IconColorFromPOV(pawn)) ?? Color.white);
				GUI.DrawTexture(new Rect(0f, position + (height - 26f) / 2f, 26f, 26f), (Texture)(object)val2);
				GUI.color = Color.white;
			}
			if (Mouse.IsOver(val))
			{
				TooltipHandler.TipRegion(val, () => log.GetTipString(), 613261 + log.LogID * 2063);
				Widgets.DrawHighlight(val);
			}
			if (Widgets.ButtonInvisible(val, log.CanBeClickedFromPOV(pawn)))
			{
				log.ClickedFromPOV(pawn);
			}
			if (DebugViewSettings.logCombatLogMouseover && Mouse.IsOver(val))
			{
