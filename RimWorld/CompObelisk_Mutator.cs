...
public class CompObelisk_Mutator : CompObelisk_ExplodingSpawner
{
	...
	private bool TryMutatingRandomAnimal(out Pawn mutatedAnimal, out Pawn resultBeast)
	{
		resultBeast = null;
		mutatedAnimal = null;
		parent.Map.mapPawns.AllPawnsSpawned.Where((Pawn pawn2) => pawn2.Faction == null && pawn2.IsNonMutantAnimal && !pawn2.Position.Fogged(parent.Map)).TryRandomElement(out var result);
		if (result == null)
		{
			return false;
		}
