...
public class InteractionWorker_RomanceAttempt : InteractionWorker
{
	...
	public override float RandomSelectionWeight(Pawn initiator, Pawn recipient)
	{
		...
		if (!new HistoryEvent(initiator.GetHistoryEventForLoveRelationCountPlusOne(), initiator.Named(HistoryEventArgsNames.Doer)).DoerWillingToDo())
		{
			...
			if (pawn != null)
			{
				float num5 = initiator.relations.OpinionOf(pawn);
				num4 = Mathf.InverseLerp(50f, -50f, num5);
			}
		}
		float num6 = (initiator.story.traits.HasTrait(TraitDefOf.Gay) ? 1f : ((initiator.gender == Gender.Female) ? 0.15f : 1f));
		float num7 = Mathf.InverseLerp(0.15f, 1f, num);
		float num8 = Mathf.InverseLerp(num2, 100f, (float)num3);
		float num9 = ((initiator.gender == recipient.gender) ? ((!initiator.story.traits.HasTrait(TraitDefOf.Gay) || !recipient.story.traits.HasTrait(TraitDefOf.Gay)) ? 0.15f : 1f) : ((initiator.story.traits.HasTrait(TraitDefOf.Gay) || recipient.story.traits.HasTrait(TraitDefOf.Gay)) ? 0.15f : 1f));
		return 1.15f * num6 * num7 * num8 * num4 * num9;
	}
	...
	private static float PartnerFactor(Pawn initiator, Pawn recipient)
	{
		...
		if (!new HistoryEvent(recipient.GetHistoryEventForLoveRelationCountPlusOne(), recipient.Named(HistoryEventArgsNames.Doer)).DoerWillingToDo())
		{
			...
			if (pawn != null)
			{
				num *= Mathf.InverseLerp(100f, 0f, (float)recipient.relations.OpinionOf(pawn));
	...
	{
		return Mathf.InverseLerp(5f, 100f, (float)recipient.relations.OpinionOf(initiator));
	}
