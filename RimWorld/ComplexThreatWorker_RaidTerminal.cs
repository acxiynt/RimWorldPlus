...
public class ComplexThreatWorker_RaidTerminal : ComplexThreatWorker
{
	...
	protected override bool CanResolveInt(ComplexResolveParams parms)
	{
		...
		{
			return ComplexUtility.TryFindRandomSpawnCell(ThingDefOf.AncientEnemyTerminal, parms.room.SelectMany((CellRect r) => r.Cells), parms.map, out spawnPosition);
		}
		return false;
	}
	...
	{
		ComplexUtility.TryFindRandomSpawnCell(ThingDefOf.AncientEnemyTerminal, parms.room.SelectMany((CellRect r) => r.Cells), parms.map, out var spawnPosition);
