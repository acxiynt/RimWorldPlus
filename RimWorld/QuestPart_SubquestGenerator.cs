...
public abstract class QuestPart_SubquestGenerator : QuestPartActivable
{
	...
	private int PendingSubquestCount => (from q in quest.GetSubquests()
	...
	public override void DoDebugWindowContents(Rect innerRect, ref float curY)
	{
		...
		if (Widgets.ButtonText(rect, "Generate " + ToString()) && CanGenerateSubquest)
		{
			GenerateSubquest();
		}
		...
		if (Widgets.ButtonText(rect2, "Remove Active " + ToString()))
		{
		...
		if (Widgets.ButtonText(rect3, "Complete " + ToString()))
		{
			Complete();
		}
