...
public class PawnColumnWorker_Carry : PawnColumnWorker
{
	...
	public override void DoCell(Rect rect, Pawn pawn, PawnTable table)
	{
		...
		if (pawn.inventoryStock != null)
		{
			...
			Widgets.Dropdown(new Rect(x, ((Rect)(ref rect)).y + 2f, (float)num2, ((Rect)(ref rect)).height - 4f), pawn, (Pawn p) => p.inventoryStock.GetDesiredThingForGroup(group), (Pawn p) => GenerateThingButtons(p, group), null, pawn.inventoryStock.GetDesiredThingForGroup(group).uiIcon, null, null, null, paintable: true);
			float num3 = x + (float)num2 + 4f;
			int num4 = Mathf.FloorToInt(num * (2f / 3f));
			Widgets.Dropdown(new Rect(num3, ((Rect)(ref rect)).y + 2f, (float)num4, ((Rect)(ref rect)).height - 4f), pawn, (Pawn p) => p.inventoryStock.GetDesiredCountForGroup(group), (Pawn p) => GenerateCountButtons(p, group), pawn.inventoryStock.GetDesiredCountForGroup(group).ToString(), null, null, null, null, paintable: true);
		}
	}
