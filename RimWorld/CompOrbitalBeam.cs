...
public class CompOrbitalBeam : ThingComp
{
	...
	public override void PostDraw()
	{
		...
		if (TicksLeft > 0)
		{
			...
			Vector3 val = Vector3Utility.FromAngleFlat(angle - 90f);
			Vector3 val2 = drawPos + val * num * 0.5f;
			val2.y = AltitudeLayer.MetaOverlays.AltitudeFor();
			float num2 = Mathf.Min((float)TicksPassed / 10f, 1f);
			Vector3 val3 = val * ((1f - num2) * num);
			...
			((Matrix4x4)(ref val4)).SetTRS(val2 + val * BeamEndHeight * 0.5f + val3, Quaternion.Euler(0f, angle, 0f), new Vector3(Props.width, 1f, num));
			Graphics.DrawMesh(MeshPool.plane10, val4, BeamMat, 0, (Camera)null, 0, MatPropertyBlock);
			Vector3 val5 = drawPos + val3;
			val5.y = AltitudeLayer.MetaOverlays.AltitudeFor();
			Matrix4x4 val6 = default(Matrix4x4);
			((Matrix4x4)(ref val6)).SetTRS(val5, Quaternion.Euler(0f, angle, 0f), new Vector3(Props.width, 1f, BeamEndHeight));
			Graphics.DrawMesh(MeshPool.plane10, val6, BeamEndMat, 0, (Camera)null, 0, MatPropertyBlock);
		}
	}
