...
public class TraitDegreeData : IRenderNodePropertiesParent
{
	...
	public List<MemeDef> GetAffectedMemes(TraitDef def, bool agreeable)
	{
		...
		if (disagreeableMemesCached == null)
		{
			...
			{
				if (!allDefsListForReading2[num].disagreeableTraits.NullOrEmpty() && allDefsListForReading2[num].disagreeableTraits.Any((TraitRequirement x) => x.def == def && (x.degree ?? 0) == degree))
				{
