...
public class MainTabsRoot
{
	...
	public void HandleLowPriorityShortcuts()
	{
		...
		if (Find.Selector.NumSelected == 0 && (int)Event.current.type == 0 && Event.current.button == 1 && !WorldRendererUtility.WorldRenderedNow && (!SteamDeck.IsSteamDeck || !Input.GetMouseButton(2)))
		{
			Event.current.Use();
