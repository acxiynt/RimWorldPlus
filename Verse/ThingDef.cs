		...
		if (race != null)
		{
			...
			if (race.body != null && race != null && tools != null)
			{
				...
				for (i2 = 0; i2 < tools.Count; i2++)
				{
					...
					{
						yield return string.Concat("has tool with linkedBodyPartsGroup ", tools[i2].linkedBodyPartsGroup, " but body ", race.body, " has no parts with that group.");
					}
				}
			}
		}
	...
	public override IEnumerable<StatDrawEntry> SpecialDisplayStats(StatRequest req)
	{
		...
		if (!verbs.NullOrEmpty())
		{
			...
			if (verb.defaultProjectile?.projectile.damageDef != null && verb.defaultProjectile.projectile.damageDef.harmsHealth)
			{
				...
				stringBuilder.AppendLine();
				float num2 = verb.defaultProjectile.projectile.GetDamageAmount(req.Thing, stringBuilder);
				yield return new StatDrawEntry(statCat, "Damage".Translate(), num2.ToString(), stringBuilder.ToString(), 5500);
		...
		for (int i = 0; i < comps.Count; i++)
		{
			foreach (StatDrawEntry item6 in comps[i].SpecialDisplayStats(req))
			{
				yield return item6;
			}
		}
		if (building != null)
		{
			if (building.mineableThing != null)
			{
				...
				yield return new StatDrawEntry(StatCategoryDefOf.Basics, "Stat_MiningYield_Name".Translate(), Mathf.CeilToInt((float)building.EffectiveMineableYield).ToString("F0"), stringBuilder3.ToString(), 2200, null, hyperlinks);
			}
		...
		if (IsMeat)
		{
			...
			yield return new StatDrawEntry(valueString: (!flag) ? string.Format("({0})", "NotYetDiscovered".Translate()) : string.Join(", ", (from thingDef in list
		...
		if (IsLeather)
		{
			...
			yield return new StatDrawEntry(valueString: (!flag2) ? string.Format("({0})", "NotYetDiscovered".Translate()) : string.Join(", ", (from thingDef in list2
				where !Find.HiddenItemsManager.Hidden(thingDef)
		...
		if (!equippedStatOffsets.NullOrEmpty())
		{
			for (int i = 0; i < equippedStatOffsets.Count; i++)
			{
				...
				float num3 = equippedStatOffsets[i].value;
				StringBuilder stringBuilder4 = new StringBuilder(stat.description);
				if (req.HasThing && stat.Worker != null)
				{
					stringBuilder4.AppendLine();
					stringBuilder4.AppendLine();
					stringBuilder4.AppendLine("StatsReport_BaseValue".Translate() + ": " + stat.ValueToString(num3, ToStringNumberSense.Offset, stat.finalizeEquippedStatOffset));
					num3 = StatWorker.StatOffsetFromGear(req.Thing, stat);
					if (!stat.parts.NullOrEmpty())
					{
						stringBuilder4.AppendLine();
						for (int num4 = 0; num4 < stat.parts.Count; num4++)
						{
							string text = stat.parts[num4].ExplanationPart(req);
							if (!text.NullOrEmpty())
							{
					...
					stringBuilder4.AppendLine();
					stringBuilder4.AppendLine("StatsReport_FinalValue".Translate() + ": " + stat.ValueToString(num3, ToStringNumberSense.Offset, !stat.formatString.NullOrEmpty()));
				}
				yield return new StatDrawEntry(StatCategoryDefOf.EquippedStatOffsets, equippedStatOffsets[i].stat, num3, StatRequest.ForEmpty(), ToStringNumberSense.Offset, null, forceUnfinalizedMode: true).SetReportText(stringBuilder4.ToString());
			}
		}
		if (!IsDrug)
		{
