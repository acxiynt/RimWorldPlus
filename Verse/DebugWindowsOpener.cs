...
public class DebugWindowsOpener
{
	...
	public void DevToolStarterOnGUI()
	{
		...
		if (Prefs.DevMode)
		{
			...
			((Vector2)(ref val))._002Ector((float)UI.screenWidth * 0.5f - widgetRowFinalX * 0.5f, 3f);
			float num = 25f;
			Find.WindowStack.ImmediateWindow(1593759361, GenUI.Rounded(new Rect(val.x, val.y, widgetRowFinalX, num)), WindowLayer.GameUI, drawButtonsCached, doBackground: false, absorbInputAroundWindow: false, 0f);
