			...
			if (localResolver == SketchResolverDefOf.Monument || localResolver == SketchResolverDefOf.MonumentRuin)
			{
				...
				for (int num = 1; num <= 60; num++)
				{
					...
					debugActionNode.AddChild(new DebugActionNode(localIndex.ToString(), DebugActionType.ToolMap)
					{
						action = delegate
						{
							RimWorld.SketchGen.SketchGen.Generate(parms: new RimWorld.SketchGen.ResolveParams
							{
								sketch = new Sketch(),
								monumentSize = new IntVec2(localIndex, localIndex)
							}, root: localResolver).Spawn(Find.CurrentMap, UI.MouseCell(), null, Sketch.SpawnPosType.Unchanged, Sketch.SpawnMode.Normal, wipeIfCollides: false, clearEdificeWhereFloor: false, null, dormant: false, buildRoofsInstantly: true);
						}
					});
				}
			}
