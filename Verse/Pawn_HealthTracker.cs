...
public class Pawn_HealthTracker : IExposable
{
	...
	private static readonly FloatRange BloodFilthDropDistanceRangeFromBleedRate;
	...
	private static List<Hediff> tmpHediffs;

	private static HashSet<Hediff> tmpRemovedHediffs;
	...
	{
		float num = capacities.GetLevel(PawnCapacityDefOf.Consciousness);
		if (InPainShock)
		{
			num -= 0.9f;
		}
		if (!(num <= 0.1f) && capacities.CanBeAwake && (capacities.CapableOf(PawnCapacityDefOf.Moving) || pawn.RaceProps.doesntMove))
		{
	...
	static Pawn_HealthTracker()
	{
		BloodFilthDropDistanceRangeFromBleedRate = new FloatRange(0.7f, 0.25f);
		tmpHediffs = new List<Hediff>(100);
		tmpRemovedHediffs = new HashSet<Hediff>();
	}
}
