...
public class HediffGiver_Heat : HediffGiver
{
	...
	public override void OnIntervalPassed(Pawn pawn, Hediff cause)
	{
		...
		if (ambientTemperature > floatRange2.max)
		{
			...
			float num = x * 6.45E-05f;
			num = Mathf.Max(num, 0.000375f);
		...
		float num3 = floatRange.max + 150f;
		if (!(ambientTemperature > num3))
		{
			return;
		}
		float x2 = ambientTemperature - num3;
		x2 = TemperatureOverageAdjustmentCurve.Evaluate(x2);
		int num4 = Mathf.Max(GenMath.RoundRandom(x2 * 0.06f), 3);
		DamageInfo dinfo = new DamageInfo(DamageDefOf.Burn, num4);
