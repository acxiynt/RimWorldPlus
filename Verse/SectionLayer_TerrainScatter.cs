...
public class SectionLayer_TerrainScatter : SectionLayer
{
	...
	public override void Regenerate()
	{
		...
		for (int num2 = cellRect.minZ; num2 <= cellRect.maxZ; num2++)
		{
			for (int num3 = cellRect.minX; num3 <= cellRect.maxX; num3++)
			{
				if (topGrid[cellIndices.CellToIndex(num3, num2)].scatterType != null)
				{
					num++;
				}
			}
		...
		while (scats.Count < num && num4 < 200)
		{
			num4++;
			...
			{
				((Vector3)(ref loc))._002Ector((float)randomCell.x + Rand.Value, (float)randomCell.y, (float)randomCell.z + Rand.Value);
				Scatterable scatterable = new Scatterable(result, loc, base.Map);
				scats.Add(scatterable);
				scatterable.PrintOnto(this);
			}
		}
		for (int num5 = 0; num5 < scats.Count; num5++)
		{
			scats[num5].PrintOnto(this);
		}
