		...
		for (int num2 = runningModsListForReading.Count - 1; num2 >= 0; num2--)
		{
			...
			for (int i = 0; i < runningModsListForReading[num2].assetBundles.loadedAssetBundles.Count; i++)
			{
				...
				if (typeof(T) == typeof(Texture2D))
				{
					...
					{
						val = (T)(object)val2.LoadAsset<Texture2D>(text + ModAssetBundlesHandler.TextureExtensions[j]);
						if (val != null)
						{
							return val;
						}
				...
				{
					val = (T)(object)val2.LoadAsset<AudioClip>(text2 + ModAssetBundlesHandler.AudioClipExtensions[k]);
					if (val != null)
					{
						return val;
					}
