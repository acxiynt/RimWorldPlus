...
public class Message : IArchivable, IExposable, ILoadReferenceable
{
	...
	public void Draw(int xOffset, int yOffset)
	{
		...
		Find.WindowStack.ImmediateWindow(Gen.HashCombineInt(ID, 45574281), rect, WindowLayer.Super, delegate
		{
			...
			{
				UIHighlighter.HighlightOpportunity(val, "Messages");
			...
			if (flashTime + 0.6f > RealTime.LastRealTime)
			{
				...
				Widgets.DrawTextHighlight(val, 0f, ColorLibrary.Yellow.ToTransparent(transparency));
			}
			Widgets.Label(new Rect(6f, 2f, ((Rect)(ref val)).width - 6f, ((Rect)(ref val)).height - 2f), text);
			if (Current.ProgramState == ProgramState.Playing && Widgets.ButtonInvisible(val))
			{
