...
public class FloatMenuWorld : FloatMenu
{
	...
	private static bool StillValid(FloatMenuOption opt, List<FloatMenuOption> curOpts, Caravan forCaravan)
	{
		...
		Vector2 cachedChoicesForPos = default(Vector2);
		((Vector2)(ref cachedChoicesForPos))._002Ector(-9999f, -9999f);
	...
	private static bool StillValid(FloatMenuOption opt, List<FloatMenuOption> curOpts, Caravan forCaravan, ref List<FloatMenuOption> cachedChoices, ref Vector2 cachedChoicesForPos)
	{
		...
		else
		{
			...
			Vector2 val = opt.revalidateWorldClickTarget.ScreenPos();
			val.y = (float)UI.screenHeight - val.y;
			List<FloatMenuOption> list;
			if (val == cachedChoicesForPos)
			{
				list = cachedChoices;
			}
			else
			{
				cachedChoices = FloatMenuMakerWorld.ChoicesAtFor(val, forCaravan);
				cachedChoicesForPos = val;
				list = cachedChoices;
			}
