...
public class LoadedObjectDirectory
{
	...
	public void RegisterLoaded(ILoadReferenceable reffable)
	{
		...
		catch (Exception ex3)
		{
			...
			Log.Error(string.Concat("Exception registering ", reffable.GetType(), " ", text5, " in loaded object directory with unique load ID ", text4, ": ", ex3));
		}
		...
		catch (Exception ex6)
		{
			...
			Log.Error(string.Concat("Exception registering ", reffable.GetType(), " ", text6, " in loaded object directory with unique thing ID ", thing.thingIDNumber, ": ", ex6));
		}
	}
