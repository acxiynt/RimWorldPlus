...
public class DrawBatch
{
	private class BatchData
	{
		...
		private static readonly Vector4 WhiteColor = Color.op_Implicit(Color.white);

		public BatchData()
		{
			matrices = (Matrix4x4[])(object)new Matrix4x4[1023];
			colors = (Vector4[])(object)new Vector4[1023];
			ptr = 0;
		}
		...
		public void Add(Matrix4x4 matrix, Color? color)
		{
			...
			matrices[ptr] = matrix;
			colors[ptr] = Color.op_Implicit((Color)(((_003F?)color) ?? Color.op_Implicit(WhiteColor)));
			ptr++;
			hasAnyColors = true;
		}
	}
	...
	private struct BatchKey : IEquatable<BatchKey>
	{
		...
		public BatchKey(Mesh mesh, Material material, int layer, bool renderInstanced, DrawBatchPropertyBlock propertyBlock)
		{
			...
			hash = ((object)mesh).GetHashCode();
			hash = Gen.HashCombineInt(hash, ((object)material).GetHashCode());
		...
		{
			if (mesh == other.mesh && material == other.material && layer == other.layer && renderInstanced == other.renderInstanced)
			{
					...
					foreach (BatchData item in batch.Value)
					{
						...
						if (draw)
						{
							...
							if (key.renderInstanced)
							{
								...
								Graphics.DrawMeshInstanced(key.mesh, 0, key.material, item.matrices, item.ptr, tmpPropertyBlock, (ShadowCastingMode)1, true, key.layer);
							}
							else
							{
								for (int i = 0; i < batchData.ptr; i++)
								{
									Matrix4x4 val = batchData.matrices[i];
									Vector4 val2 = batchData.colors[i];
									if (batchData.hasAnyColors)
									{
										tmpPropertyBlock.SetColor("_Color", Color.op_Implicit(val2));
									}
									Graphics.DrawMesh(key.mesh, val, key.material, key.layer, (Camera)null, 0, tmpPropertyBlock);
								}
							}
						}
						batchData.Clear();
