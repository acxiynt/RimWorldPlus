...
public class PsychicRitualToil_SkipAbductionPlayer : PsychicRitualToil
{
	...
	private void ApplyOutcome(PsychicRitual psychicRitual, Pawn invoker)
	{
		...
		bool flag = false;
		Pawn[] source = psychicRitual.Map.attackTargetsCache.TargetsHostileToColony.Where((IAttackTarget t) => t.Thing is Pawn pawn2 && pawn2.RaceProps.Humanlike && !pawn2.IsMutant && pawn2.Faction != Faction.OfPlayer && !t.ThreatDisabled(invoker) && !pawn2.IsOnHoldingPlatform).Cast<Pawn>().ToArray();
		Pawn pawn = null;
