...
public class PathFinder
{
	...
	private static ushort statusOpenValue;

	private static ushort statusClosedValue;
	...
	private static readonly SimpleCurve NonRegionBasedHeuristicStrengthHuman_DistanceCurve;
	...
	public PawnPath FindPath(IntVec3 start, LocalTargetInfo dest, TraverseParms traverseParms, PathEndMode peMode = PathEndMode.OnCell, PathFinderCostTuning tuning = null)
	{
		...
		int element;
		int priority;
		while (openList.TryDequeue(out element, out priority))
		{
			curIndex = element;
			if (priority != calcGrid[curIndex].costNodeCost || calcGrid[curIndex].status == statusClosedValue)
			{
				continue;
			}
			...
			for (int i = 0; i < 8; i++)
			{
				...
				if ((ulong)num8 >= (ulong)mapSizeX || (ulong)num9 >= (ulong)mapSizeZ)
				{
					continue;
				}
				int num10 = (int)num8;
				...
				int num19 = Mathf.RoundToInt((float)(num16 + calcGrid[num12].heuristicCost));
				if (num19 < 0)
				{
	...
	static PathFinder()
	{
		...
		NonRegionBasedHeuristicStrengthHuman_DistanceCurve = new SimpleCurve
		{
			new CurvePoint(40f, 1f),
			new CurvePoint(120f, 2.8f)
		};
		RegionHeuristicWeightByNodesOpened = new SimpleCurve
		{
	...
	public async Task<PawnPath> FindPathAsync(IntVec3 start, LocalTargetInfo dest, TraverseParms traverseParms, PathEndMode peMode = PathEndMode.OnCell, PathFinderCostTuning tuning = null)
	{
		if (DebugSettings.pathThroughWalls)
		{
			traverseParms.mode = TraverseMode.PassAllDestroyableThings;
