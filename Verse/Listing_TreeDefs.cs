...
public class Listing_TreeDefs : Listing_Tree
{
	...
	protected unsafe void ValueEditWidgetRight(TreeNode_Editor node, float leftX)
	{
		...
		else if (objectType == typeof(FloatRange))
		{
			...
			Widgets.FloatRangeWithTypeIn(rect, ((object)(*(Rect*)(&rect))/*cast due to .constrained prefix*/).GetHashCode(), ref fRange, sliderMin, sliderMax);
			obj = fRange;
		}
		else if (objectType == typeof(IntRange))
		{
			...
			Widgets.IntRange(rect, ((object)(*(Rect*)(&rect))/*cast due to .constrained prefix*/).GetHashCode(), ref range, min, max);
			obj = range;
		}
		else
		{
