...
public class DesignationDragger
{
	...
	public void DraggerOnGUI()
	{
		...
		if (SelDes.DragDrawOutline && (intVec.x > 1 || intVec.z > 1))
		{
			...
			((Vector3)(ref v))._002Ector((float)Mathf.Min(startDragCell.x, intVec2.x), 0f, (float)Mathf.Min(startDragCell.z, intVec2.z));
			Vector3 v2 = new Vector3((float)(Mathf.Max(startDragCell.x, intVec2.x) + 1), 0f, (float)(Mathf.Max(startDragCell.z, intVec2.z) + 1));
