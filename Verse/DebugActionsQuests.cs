...
public static class DebugActionsQuests
{
	...
	{
		Action<QuestScriptDef, Slate> generateQuest = delegate(QuestScriptDef script, Slate slate2)
		{
			int num = 0;
		...
		Func<QuestScriptDef, Slate, bool, List<DebugActionNode>> selectPoints = delegate(QuestScriptDef script, Slate slate2, bool generate)
		{
			...
			foreach (float item in DebugActionsUtility.PointsOptions(extended: false))
			{
				...
				if (script != null)
				{
					if (script.IsRootDecree)
					{
						slate2.Set("asker", PawnsFinder.AllMapsCaravansAndTravelingTransportPods_Alive_FreeColonists.RandomElement());
					}
				...
				list2.Add(new DebugActionNode(text, DebugActionType.Action, delegate
				{
					...
					if (generate)
					{
						generateQuest(script, slate2);
					}
				}));
			}
			return list2;
		};
		Func<QuestScriptDef, Slate, bool, List<DebugActionNode>> selectPopulation = delegate(QuestScriptDef script, Slate slate2, bool generate)
		{
			List<DebugActionNode> list2 = new List<DebugActionNode>
			{
		...
		foreach (QuestScriptDef item3 in DefDatabase<QuestScriptDef>.AllDefs.Where((QuestScriptDef x) => x.IsRootAny))
		{
			...
			if (localScriptDef.affectedByPoints && localScriptDef.affectedByPopulation)
			{
				...
				{
					List<DebugActionNode> list2 = new List<DebugActionNode>();
					foreach (DebugActionNode item4 in selectPoints(localScriptDef, slate, arg3: false))
					{
						item4.childGetter = () => selectPopulation(localScriptDef, slate, arg3: true);
						list2.Add(item4);
					}
					return list2;
				};
			...
			{
				debugActionNode.childGetter = () => selectPoints(localScriptDef, slate, arg3: true);
			}
			else if (localScriptDef.affectedByPopulation)
			{
				debugActionNode.childGetter = () => selectPopulation(localScriptDef, slate, arg3: true);
			}
			else
			{
