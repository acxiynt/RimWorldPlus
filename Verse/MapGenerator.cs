	...
	public static Map GenerateMap(IntVec3 mapSize, MapParent parent, MapGeneratorDef mapGenerator, IEnumerable<GenStepWithParams> extraGenStepDefs = null, Action<Map> extraInitBeforeContentGen = null, bool isPocketMap = false)
	{
		...
		try
		{
			...
			IEnumerable<GenStepWithParams> enumerable = from x in mapGenerator.genSteps
				where !Find.Scenario.parts.Any((ScenPart p) => typeof(ScenPart_DisableMapGen).IsAssignableFrom(p.def.scenPartClass) && p.def.genStep == x)
		...
		try
		{
			...
			for (int num = 0; num < tmpGenSteps.Count; num++)
			{
				...
				try
				{
					...
					tmpGenSteps[num].def.genStep.Generate(map, tmpGenSteps[num].parms);
				}
				catch (Exception ex)
				{
