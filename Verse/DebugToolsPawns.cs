...
public static class DebugToolsPawns
{
	...
	private static void AddGuest(GuestStatus guestStatus)
	{
		foreach (Building_Bed item in Find.CurrentMap.listerBuildings.AllBuildingsColonistOfClass<Building_Bed>())
		{
			...
			PawnKindDef pawnKindDef = ((guestStatus != GuestStatus.Guest) ? DefDatabase<PawnKindDef>.AllDefs.Where((PawnKindDef pk) => pk.defaultFactionType != null && !pk.defaultFactionType.isPlayer && pk.RaceProps.Humanlike && pk.mutant == null).RandomElement() : PawnKindDefOf.SpaceRefugee);
