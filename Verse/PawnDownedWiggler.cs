...
public class PawnDownedWiggler
{
	...
	{
		int num = 0;
		int num2 = wigglePeriodTick - ticksPassed;
		int num3 = Mathf.Min(90, wigglePeriodTick) - Mathf.Max(num, num2);
		if (num3 > 0)
		{
			downedAngle += 0.35f * (float)num3 * (float)(positiveAngleWiggle ? 1 : (-1));
		}
	}
