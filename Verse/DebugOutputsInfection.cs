...
public static class DebugOutputsInfection
{
	...
	public static void Infections()
	{
		...
		{
			float num = (isAnimal(d) ? 1f : ThingDefOf.Bed.GetStatValueAbstract(StatDefOf.ImmunityGainSpeedFactor));
			float num2 = Mathf.Lerp(1f, num, bedridden ? 1f : 0.3f) * StatDefOf.ImmunityGainSpeed.GetStatPart<StatPart_Resting>().factor;
