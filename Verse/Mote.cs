...
public abstract class Mote : Thing
{
	...
	public override Vector3 DrawPos
	{
		get
		{
			...
			{
				num = (Mathf.Cos(Mathf.Clamp01((AgeSecs + def.mote.archStartOffset) / def.mote.archDuration) * (float)Math.PI * 2f - (float)Math.PI) + 1f) / 2f * def.mote.archHeight;
			}
			...
			float num3 = (float)Mathf.Abs(num2) / 2.1474836E+09f * (1f / 26f) * def.mote.yFightingOffsetScalar01;
