...
public class Noise2D : IDisposable
{
	...
	public Texture2D GetTexture(Gradient gradient)
	{
		...
		Texture2D val = new Texture2D(m_width, m_height);
		((Object)val).name = "Noise2DTex";
		Color[] array = (Color[])(object)new Color[m_width * m_height];
		for (int i = 0; i < m_width; i++)
		{
			for (int j = 0; j < m_height; j++)
			{
	...
	public Texture2D GetNormalMap(float intensity)
	{
		...
		for (int i = 0; i < m_ucWidth; i++)
		{
			for (int j = 0; j < m_ucHeight; j++)
			{
				...
				Vector3 val2 = new Vector3(num * intensity, 0f, 1f);
				((Vector3)(ref val3))._002Ector(0f, num2 * intensity, 1f);
