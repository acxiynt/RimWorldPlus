...
public class EditWindow_DebugInspector : EditWindow
{
	...
	public override void DoWindowContents(Rect inRect)
	{
		...
		for (int num4 = 0; num4 < array.Length; num4++)
		{
			...
			if (num2 + num5 > ((Rect)(ref inRect)).yMax)
			{
				num3 += num;
	...
	private string CurrentDebugString()
	{
		...
		if (Current.ProgramState == ProgramState.Playing && Find.CurrentMap != null)
		{
			...
			if (intVec.InBounds(Find.CurrentMap))
			{
				...
				if (DebugViewSettings.writeAttackTargets)
				{
					foreach (Pawn item in Find.CurrentMap.thingGrid.ThingsAt(UI.MouseCell()).OfType<Pawn>())
					{
						...
						for (int num = 0; num < potentialTargetsFor.Count; num++)
						{
							...
							stringBuilder.AppendLine(string.Concat(thing.LabelShort, ", ", thing.Faction, potentialTargetsFor[num].ThreatDisabled(null) ? " (threat disabled)" : ""));
						}
					}
				}
				...
				{
					stringBuilder.AppendLine("---");
					int num3 = Find.CurrentMap.pathing.Normal.pathGrid.PerceivedPathCostAt(intVec);
					...
					int num5 = Find.CurrentMap.pathing.FenceBlocked.pathGrid.PerceivedPathCostAt(intVec);
